...
namespace render {
	...
	{
		rootSignature_ = getRootSignature(type, device);
	}
	...
	PipelineStateDx12::PipelineStateDx12(GraphicsDevice* device, const PipelineStateDescriptor& descriptor)
		: PipelineState(device), descriptor_(descriptor)
	{
	}

	void PipelineStateDx12::init()
	{
		D3D12_GRAPHICS_PIPELINE_STATE_DESC psoDesc;
		ZeroMemory(&psoDesc, sizeof(D3D12_GRAPHICS_PIPELINE_STATE_DESC));
		psoDesc.InputLayout = { descriptor_.inputElement_.data(), (UINT)descriptor_.inputElement_.size() };
		psoDesc.pRootSignature = static_cast<RootSignatureDx12*>(descriptor_.rootSignature_.get())->raw();
		...
		ThrowIfFailed(static_cast<DeviceDx12*>(parent())->device()->CreateGraphicsPipelineState(&psoDesc, IID_PPV_ARGS(&pipelineState_)));
	}
